var DFT = function($) {
    ...
    var bindClick = function() {
        ...
        $(document).on('click', '.btn-detail', function() {
            ...
            _oldTicketLeft.fadeOut(function() {
                _Ajax('ticket-mail-edit', 'POST', [{ _id: button.attr('data-id') }], function(resp) {
        ...
        $(document).on('click', '.zpaging', function() {
            ...
            _Ajax(queryLink, 'GET', {}, function(resp) {
                if (resp.code == 200) {
                    ...
                    $("#frm-edit-ticket-customer-mail").html(zoka.showTicketListCustomerJourney(null, resp.message.data, true));
                    $(".paging-list-mail").html(zoka.createPaging(resp.message.paging));
        ...
        $(document).on('click', '#searchTicket', function() {
            ...
            var searchObj = '/ticket-mail-edit?search=1&idCustomer=' + $(this).attr('data-id') + '&tId=' + $(this).attr('data-ticket-id');
        ...
        $(document).on('click', '.clickToCall', function() {
            ...
            if (callNumber) {
                ...
                if ((currentTicket && currentTicket.idService) || currentTicket && currentTicket.idMailInboundChannel) {
                    // là ticket gọi vào - confirm tạo ticket
                    swal({
                        title: "Thông báo",
                        text: "Bạn đang gọi ra trên ticket  mail!",
                        type: "warning",
        ...
        // Cập nhật giao diện khi chọn câu hỏi khảo sát tiếp theo
            ...
            _.each($('.survey-tab'), function(el) {
                if ($(el).hasClass('active')) {
                    ...
                    if (nextQuestion) {
                        ...
                        if ($('.' + nextQuestion).attr('data-next-question')) {
                            $('.btn-next').show();
                        } else {
                            $('.btn-next').hide();
                        }
        ...
        // Hiển thị dữ liệu câu hỏi khảo sát
    ...
    var bindSubmit = function() {
        ...
        $('#frm-update-customer').validationEngine('attach', {
            ...
            onValidationComplete: function(form, status) {
                ...
                if (status) {
                    _AjaxData('/ticket-mail-edit/customer-' + $('#save-customer').attr('data-id') + query, 'PUT', $(form).getData(), function(resp) {
                        if (resp.code == 200) {
                            swal({
        ...
        $('#frm-edit-ticket').validationEngine('attach', {
            ...
            onValidationComplete: function(form, status) {
                if (status) {
                    _AjaxData('/ticket-mail-edit/editTicket-' + $('#save-new-ticket').attr('data-id'), 'PUT', $(form).getData(), function(resp) {
        ...
        $('#frm-ticket-detail-crm').validationEngine('attach', {
            // Xác nhận cập nhật thông tin ticket liên quan
            validateNonVisibleFields: true,
            autoPositionUpdate: true,
            validationEventTrigger: 'keyup',
            onValidationComplete: function(form, status) {
                if (status) {
                    _AjaxData('/ticket-mail-edit/editTicket-' + $('#save-ticket-detail-crm').attr('data-id'), 'PUT', $(form).getData(), function(resp) {
                        if (resp.code == 200) {
                            swal({
                                title: 'Cập nhật thành công',
                                text: '',
        ...
        // Xác nhận cập nhật thông tin câu hỏi khảo sát
        $('#current-survey-form').validationEngine('attach', {
            ...
            onValidationComplete: function(form, status) {
                if (status) {
                    ...
                    _AjaxData('/ticket-mail-edit/survey-' + dataLink, 'PUT', $(form).getData(), function(resp) {
                        if (resp.code == 200) {
        ...
        // Xác nhận cập nhật thông tin câu hỏi khảo sát ticket liên quan
    ...
    // Cập nhật dữ liệu tình trạng ticket khi thay đổi
    ...
    // Khóa chức năng cập nhật câu hỏi khảo sát
