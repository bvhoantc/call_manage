var DFT = (function ($) {
  ...
  var bindClick = function () {
    ...
    $(document).on("click", ".delete-restaurant", handleEventClickDeleteRestaurant);
  ...
  var bindChange = function () {
    ...
    function handleEventChangeCheckBoxRestaurants(e) {
      ...
      if (target.val() === "all") {
        typeSearch = cbNotAll
          .not('[value="all"]')
          .map((index, item) => JSON.parse(decodeURIComponent($(item).attr("data-json"))))
          .get();
        if (target.is(":checked")) {
          // thêm hết
        ...
        typeSearch = JSON.parse(decodeURIComponent(target.attr("data-json")));
        if (target.is(":checked")) {
            ...
            [
              ...
              `<td class="text-center">
                        <span><input name="checkbox-restaurants" type="checkbox" value="${
                          i._id
                        }" data-json=\'${encodeURIComponent(JSON.stringify(i))}\'></span>
                    </td>`,
            ].join("") +
            "</tr>"
          );
            ...
            let colArea = ele.map(j => {
                ...
                return "<tr>" + [
                    ...
                    `<td class="${_class} text-center">
                        <i class="zmdi zmdi-close-circle text-danger delete-restaurant" data-id="${j._id}"></i>
                    </td>`,
                ].join("") + "</tr>"
            }).join("");
  ...
  return {
    ...
    uncut: function () {
      ...
      $(document).off("change", ".delete-restaurant");
      $(document).off("change", ".delete-file");
    },
  };
})(jQuery);
