var DFT = (function ($) {
  ...
  var bindClick = function () {
    ...
    $(document).on("click", "#btnUpdatePromotions", handleEventClickBtnUpdatePromotions);
    $(document).on("click", ".delete-restaurant-selected", handleEventClickDeleteRestaurant);
    $(document).on("click", ".delete-file", handleEventClickDeleteFile);
    
    function handleEventClickBtnUpdatePromotions(e) {
      e.preventDefault();
      let target = $(e.currentTarget);
      let promotionID = target.attr("data-id");
      let data = {
        title: $("#title").val(),
      ...
      if(!promotionID) return swal({title: "không thể thực hiện lúc này!"});
      ...
      // return console.log(formData, filesToUpload);
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.code != 200) {
            ...
            swal({
              ...
              window.location.hash = "#promotions-manager";
            });
    ...
    function handleEventClickDeleteRestaurant(e) {
      e.preventDefault();
      let target = $(e.currentTarget);
      let eleID = target.attr("data-id");
      RestaurantSelected = RestaurantSelected.filter(i => i._id != eleID);
    ...
    function handleEventClickDeleteFile(e) {
      ...
      if(fileObjectID){
        _AjaxDataCustom(`/promotions/`+ promotionCurrentID, "PUT", JSON.stringify({deleteFileID: fileObjectID}), {
  ...
  function renderProvinces(data, type) {
    ...
    selected = ""; // do test và BA ko thích chọn "cả nước" mà auto chọn cả 63 tỉnh
    $("#province").html(
      data
        .map(
          (i) =>
    ...
    $(`#tblRestaurant tbody`).html(
      data
        .map((i, index) => {
          ...
          let provinceName = "";
          let checked = "";
          let restaurantFound = RestaurantSelected.find(j => {
              return j._id == i._id;
          });
          ...
          if(restaurantFound)  checked = "checked";
          ...
          return (
            ...
            [
              ...
              `<td class="text-center">
                        ...
                        <input 
                          ${checked}
                          name="checkbox-restaurants"
                          type="checkbox"
                          value="${i._id}"
                          data-json=\'${JSON.stringify(i)}\'>
                        </span>
                    </td>`,
            ].join("") +
            "</tr>"
        ...
        Object.keys(dataRender).map((i, index) => {
            ...
            let colArea = ele.map(j => {
                ...
                return "<tr>" + [
                    ...
                    `<td class="${_class} text-center">
                        <span>
                          <i 
                          data-id="${j._id}" data-json=\'${JSON.stringify(j)}\'
                          class="zmdi zmdi-close-circle text-danger delete-restaurant-selected"></i>
                    </td>`,
                ].join("") + "</tr>"
            }).join("");
      ...
      Object.keys(files)
        .map((i, index) => {
          ...
          return (
            ...
            [
              `<td>Tên tài liệu</td>`,
              `<td class="text-center">${moment(i.lastModifiedDate || i.created).format(
                "HH:mm DD/MM/YYYY"
              )}</td>`,
              `<td>${files[i].name}</td>`,
  ...
  return {
    init: function () {
      ...
      renderStatus(
        "create_promotion",
        $("#create_promotion_status").is(":checked")
      );
      ...
      RestaurantSelected = restaurantsData;
      $("#startTime").val(moment(promotionStartTime).format("DD/MM/YYYY"));
      $("#endTime").val(moment(promotionEndTime).format("DD/MM/YYYY"));
    ...
    uncut: function () {
      ...
      $(document).off("click", "#btnUpdatePromotions");
      ...
      $(document).off("click", ".delete-restaurant-selected");
